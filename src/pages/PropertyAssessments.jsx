...
const PropertyAssessments = () => {
  ...
  return (
    <div className="flex gap-6 relative items-start h-full pb-8">
      ...
      {/* Main Content */}
      <div className={`flex flex-col space-y-6 transition-all duration-300 ${showPreview ? 'w-[calc(100%-420px)]' : 'w-full'}`}>

        {/* Stats */}
        <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <p className="text-[11px] font-bold text-gray-500 mb-1">Total Applications</p>
            <h4 className="text-2xl font-black text-[#081326]">{applications.length}</h4>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <p className="text-[11px] font-bold text-gray-500 mb-1">Nagar Nigam</p>
            <h4 className="text-2xl font-black text-indigo-500">{applications.filter(a => a.serviceType === 'nagar-nigam').length}</h4>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <p className="text-[11px] font-bold text-gray-500 mb-1">LDA Map</p>
            <h4 className="text-2xl font-black text-teal-500">{applications.filter(a => a.serviceType === 'lda-map').length}</h4>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            <p className="text-[11px] font-bold text-gray-500 mb-1">Map Estimate</p>
            <h4 className="text-2xl font-black text-amber-500">{applications.filter(a => a.serviceType === 'map-estimate').length}</h4>
          </div>
        </div>
      ...
      {showPreview && selectedApp && (
        <div className="w-[410px] flex-shrink-0 sticky top-0">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col max-h-[calc(100vh-80px)]">
            {/* Header */}
            <div className="p-5 border-b border-gray-100 flex items-start justify-between bg-[#081326] text-white">
              <div>
